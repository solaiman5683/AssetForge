...
async function downloadAllFiles() {
    ...
    showMessage('Saving all files...', 'info');
    
    for (const result of processedResults) {
        const fileName = `optimized_${Date.now()}.${result.format || 'jpg'}`;
        await window.electronAPI.saveFile(fileName, result.buffer);
        await new Promise(resolve => setTimeout(resolve, 300));
    }
    
    showMessage(`Saved ${processedResults.length} file(s)!`, 'success');
}
